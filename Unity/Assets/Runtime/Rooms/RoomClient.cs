...
namespace Ubiq.Rooms
{
	...
	public class RoomClient : MonoBehaviour
	{
		...
		/// </summary>
		[SerializeField] private ConnectionDefinition[] servers;
		...
		public class TimeoutNotification : Notification
		{
			...
			{
				get { return $"No Connection ({client.heartbeatReceived.ToString("0")} seconds ago)"; }
			}
		}
		...
		public bool JoinedRoom
		{
			get
			{
				return Room != null && Room.UUID != null &&
				       Room.UUID != ""; // Null check because this may be called in the Editor
			}
		}
		...
		protected void ProcessMessage(ReferenceCountedSceneGraphMessage message)
		{
			...
			switch (container.type)
			{
				...
				case "Rooms":
				{
					...
					{
						Debug.LogError(
							$"Your version {roomClientVersion} of Ubiq doesn't match the server version {args.version}.");
					}
		...
		private void
			SetBlob(string room, string uuid,
				string blob) // private because this could encourage re-using uuids, which is not allowed because blobs are meant to be immutable
		{
			if (blob.Length > 0)
			{
		...
		private void OnPingResponse(PingResponseArgs args)
		{
			...
			{
				Join(name: "",
					publish: false); // The RoomClient has re-established connectivity with the RoomServer, but under a different state. So, leave the room and let the user code re-establish any state.
			}
