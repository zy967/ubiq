...
namespace Ubiq.Messaging
{
	...
	public struct ReferenceCountedSceneGraphMessage : IReferenceCounter
	{
		...
		{
			get { return new Span<byte>(bytes, start, length); }
		}

		public NetworkId objectid
		{
			get { return new NetworkId(buffer.bytes, buffer.start); }
			set { value.ToBytes(buffer.bytes, buffer.start); }
		}
	...
	{
		[SerializeField] private UInt32 a;
		[SerializeField] private UInt32 b;
		...
		public NetworkId(string id)
		{
			...
			ulong numeric;
			id = id.Replace("-", "");
			if (ulong.TryParse(id, System.Globalization.NumberStyles.HexNumber,
			...
			if (ulong.TryParse(id, System.Globalization.NumberStyles.AllowHexSpecifier,
				    System.Globalization.CultureInfo.CurrentCulture, out numeric))
			{
				a = (uint) (numeric >> 32);
				b = (uint) (numeric & 0xffffffffL);
		...
		{
			a = System.Buffers.Binary.BinaryPrimitives.ReadUInt32LittleEndian(
				new ReadOnlySpan<byte>(buffer, offset + 0, 4));
			b = System.Buffers.Binary.BinaryPrimitives.ReadUInt32LittleEndian(
				new ReadOnlySpan<byte>(buffer, offset + 4, 4));
		}
