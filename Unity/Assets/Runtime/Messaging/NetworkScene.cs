...
namespace Ubiq.Messaging
{
	...
	public class NetworkScene : MonoBehaviour
	{
		...
		private Dictionary<NetworkId, List<Action<ReferenceCountedSceneGraphMessage>>> processorCollections =
			new Dictionary<NetworkId, List<Action<ReferenceCountedSceneGraphMessage>>>();
			...
			if (transform.parent == null)
			{
				...
				else // Only one networkscene can exist at the top level of the hierarchy
				{
					gameObject.SetActive(
						false); // Deactivate the branch to avoid Start() being called until the branch is destroyed
					Destroy(gameObject);
				}
			}
